...
import { useProposalChatModal } from './index'
...
const {
  ...
  Loader2,
  Send
} = useProposalChatModal(props, emit)
</script>
...
<template>
  <BaseDialog
    ...
    size="lg"
  >
    <div v-if="proposal" class="p-0 flex flex-col md:h-[65vh] h-[80vh] bg-[#E5DDD5] dark:bg-gray-950 rounded-b-[0.5rem] overflow-hidden proposal-chat-modal-container">
      <!-- Header -->
      ...
      <div ref="chatMessagesRef" class="flex-1 overflow-y-auto p-6 space-y-4 bg-[#dfe4ea] dark:bg-gray-900/80 scrollbar-hide">
        <div v-if="!groupedMessages?.length" class="text-center py-20 bg-white/60 dark:bg-gray-900/60 backdrop-blur-sm rounded-[.5rem] p-8 max-w-xs mx-auto mt-10">
          <div class="w-16 h-16 bg-white dark:bg-gray-800 rounded-[.5rem] flex items-center justify-center mx-auto mb-4 border border-gray-100 dark:border-gray-700 shadow-sm">
        ...
        <div v-for="group in groupedMessages" :key="group.date" class="space-y-4">
          ...
          <div class="flex justify-center my-6">
            <span class="px-4 py-1.5 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-[.5rem] text-[9px] font-black text-gray-500 dark:text-gray-300 uppercase tracking-widest shadow-sm">
              {{ group.date }}
            </span>
          </div>
          ...
          >
            ...
            <div
              :class="[
                'px-4 py-2.5 rounded-[.5rem] text-sm font-medium leading-relaxed shadow-sm min-w-[80px]',
        ...
        <form @submit.prevent="sendMessage" class="flex gap-3 items-center">
          <div class="flex-1 relative">
            <input
              ...
              class="w-full px-6 py-3.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-emerald-500/20 rounded-[.5rem] outline-none font-medium text-sm shadow-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500"
            />
          </div>
          <button
            type="submit"
