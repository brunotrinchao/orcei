  ...
  <DialogRoot :open="open" @update:open="handleOpenUpdate">
    <DialogPortal>
      ...
      >
        ...
        <div class="absolute -top-40 -left-40 w-96 h-96 bg-gradient-to-tr from-indigo-500/15 via-blue-500/10 to-transparent rounded-full blur-3xl pointer-events-none animate-pulse duration-[8000ms]" />
        ...
        <div class="relative px-6 py-5 border-b border-gray-100 dark:border-slate-800/80 flex items-center justify-between z-10">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-2xl bg-gradient-to-tr from-indigo-600 to-blue-600 flex items-center justify-center shadow-lg shadow-indigo-500/20">
          ...
          <DialogClose 
            ...
            @click="close" 
            :disabled="loading"
            class="p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer disabled:opacity-50"
          >
            <X class="w-5 h-5" />
          </DialogClose>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-6 space-y-6 z-10 custom-scrollbar">
          ...
          <div class="p-5 bg-gradient-to-br from-indigo-50/80 to-blue-50/40 dark:from-indigo-950/30 dark:to-blue-950/20 border border-indigo-100 dark:border-indigo-900/40 rounded-3xl space-y-3">
            <div class="flex items-center justify-between">
              ...
              <span class="text-[10px] font-black text-indigo-800 dark:text-indigo-200 uppercase bg-indigo-100/80 dark:bg-indigo-900/50 px-3 py-0.5 rounded-full border border-indigo-200 dark:border-indigo-800">
                {{ currentPeriodObj.fullLabel }}
              </span>
            </div>

            <!-- Botões Seletores de Período (Interativos) -->
            <div v-if="allowChangePeriod" class="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-hide bg-indigo-100/40 dark:bg-indigo-950/50 p-1.5 rounded-2xl border border-indigo-200/40 dark:border-indigo-800/40">
              <button
                ...
                :class="[
                  'px-3.5 py-1.5 rounded-xl text-[10px] uppercase tracking-wider font-black transition-all cursor-pointer whitespace-nowrap',
          ...
          <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <div class="bg-gray-50 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-gray-100 dark:border-slate-800 space-y-1 col-span-2 sm:col-span-2">
          ...
          <div class="p-5 bg-slate-900 dark:bg-slate-950 text-white rounded-3xl border border-white/10 shadow-xl space-y-3 relative overflow-hidden">
        ...
        <div class="px-6 py-4 border-t border-gray-100 dark:border-slate-800/80 flex items-center justify-between gap-4 z-10 bg-gray-50/50 dark:bg-slate-900/50">
          <BaseButton 
            ...
            @click="close" 
            :disabled="loading"
            class="px-5 py-3 rounded-xl text-xs font-bold uppercase tracking-wider"
          >
            Cancelar
          </BaseButton>
          ...
          <BaseButton 
            ...
            @click="handleConfirm" 
            :disabled="loading"
            class="flex-1 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white rounded-xl py-3 text-xs font-black tracking-widest uppercase shadow-lg shadow-indigo-600/20 border border-indigo-400/20 cursor-pointer disabled:opacity-50"
          >
