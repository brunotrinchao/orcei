...
<template>
  <section id="endereco" data-tour="config-endereco" class="bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] border border-gray-200 dark:border-gray-800 shadow-sm scroll-mt-8">
    <div class="flex items-center gap-3 mb-8">
      <div class="w-10 h-10 rounded-xl bg-orange-50 dark:bg-orange-950/50 flex items-center justify-center">
        <MapPin class="w-5 h-5 text-orange-600 dark:text-orange-400" />
      </div>
      <h2 class="text-xl font-black text-gray-900 dark:text-gray-50 uppercase tracking-tight">Endereço Comercial</h2>
    </div>
      ...
      <div class="space-y-3">
        ...
        <div class="relative">
          ...
          <div v-if="isSearchingCEP" class="absolute right-4 top-1/2 -translate-y-1/2">
            <RefreshCcw class="w-5 h-5 animate-spin text-blue-600 dark:text-blue-400" />
          </div>
        </div>
      </div>
