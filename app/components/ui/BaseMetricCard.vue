...
const colorStyles: Record<string, { iconBg: string; badge: string }> = {
  green: {
    iconBg: 'bg-green-50 dark:bg-green-950/30 text-green-600 dark:text-green-400',
...
<template>
  ...
  <div v-if="loading"
    class="bg-white dark:bg-gray-900 p-4 md:p-5 rounded-[0.75rem] border border-slate-200 dark:border-gray-800 shadow-sm shadow-slate-200/50 dark:shadow-none flex flex-row md:flex-col items-center md:items-start justify-between md:justify-start gap-3 md:gap-0">
    <div class="flex flex-row justify-between w-auto md:w-full items-center md:items-start mb-0 md:mb-4 shrink-0">
      <div class="flex items-center md:items-start flex-1 min-w-0">
        <BaseSkeleton
          ...
          customClass="md:w-12 md:h-12 w-10 h-10"
        />
  ...
  <div v-else
    ...
    <div class="group flex flex-row md:flex-col items-center md:items-start justify-between md:justify-start gap-3 md:gap-0">
      <div class="flex flex-row justify-between w-auto md:w-full items-center md:items-start mb-0 md:mb-4 shrink-0">
        <div class="flex items-center md:items-start flex-1 min-w-0">
          <div v-if="cardIcon"
            class="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-[0.75rem] flex items-center justify-center transition-colors shrink-0"
            :class="computedColorStyle.iconBg">
            <component :is="cardIcon" class="w-5 h-5 md:w-6 md:h-6" />
          </div>
          <div class="ml-2.5 md:ml-3 min-w-0">
            <p v-if="cardTitle"
              class="text-[10px] sm:text-xs md:text-sm font-bold text-gray-700 dark:text-gray-400 uppercase tracking-wider mb-0 md:mb-1 truncate">
              {{ cardTitle }}
            </p>
            <p v-if="cardSubtitle"
              class="hidden md:block text-[11px] text-slate-500 dark:text-gray-500 mt-0.5 font-medium truncate">
              {{ cardSubtitle }}
            </p>
          </div>
        </div>
        ...
        <span v-if="badge"
          class="hidden md:inline-block text-[9px] font-semibold tracking-wide px-2.5 py-1 rounded-lg border shrink-0 ml-2"
          :class="computedColorStyle.badge">
          {{ badge }}
        </span>
      </div>

      <div class="flex-1 min-w-0 w-full text-right md:text-left flex flex-col justify-center items-end md:items-start">
      <h3 v-if="value !== undefined && value !== null"
        class="text-base sm:text-lg md:text-3xl font-black text-gray-900 dark:text-white tracking-tight truncate">
        {{ value }}
      </h3>
