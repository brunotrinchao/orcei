...
const bgClasses = computed(() => {
  ...
  switch (bg) {
    case "slate":
    case "slate-100":
      return "bg-slate-800 dark:bg-slate-900 text-slate-300 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500";
    case "slate-900":
    case "slate-950":
...
const sizeClasses = computed(() => {
  switch (props.size) {
    case "xs":
      return "p-2 text-xs rounded-md";
    case "sm":
      return "p-2.5 text-xs rounded-lg";
    case "lg":
      return "p-4 text-base rounded-[0.75rem]";
    case "xl":
      return "p-5 text-lg rounded-xl";
    case "2xl":
      return "p-6 text-xl rounded-2xl";
    case "md":
    default:
      if (
        props.size &&
        (props.size.startsWith("p-") || props.size.startsWith("text-"))
      ) {
        return props.size;
      }
      return "p-3.5 text-sm rounded-[0.75rem]";
  }
});
...
const isEmpty = () =>
  !!props.required && (!props.modelValue || props.modelValue.trim() === "");
...
<template>
  <div class="space-y-2">
    ...
    <textarea
      ...
      :required="required"
      :class="[
        'w-full transition-all outline-none font-bold disabled:opacity-50 disabled:bg-gray-50 dark:disabled:bg-gray-900 disabled:cursor-not-allowed shadow-xs resize-none',
        borderClasses,
        bgClasses,
        sizeClasses,
        displayError
    ...
    <span
      class="block text-[10px] font-bold text-right"
      v-if="effectiveMaxLength > 0"
      :class="
