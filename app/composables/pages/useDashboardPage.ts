...
import { getPhaseColorHex } from "~/utils/proposalLifecycle";
import {
  Chart as ChartJS,
  Title,
  Tooltip,
...
export function useDashboardPage() {
  ...
  const { data: profile } = useLazyFetch<any>("/api/profile", {
    key: "profile",
  });
  ...
  const actionCostsList = computed(() => [
    {
      ...
      description:
        "Notificação por e-mail e disponibilização de link público rastreável com notificações em tempo real",
      icon: FileText,
      badge: "Comercial",
      badgeColor:
        "bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400 border-blue-100 dark:border-blue-900/40",
    },
    {
      key: "proposalSuggest",
      name: "Assistente de Orçamentos IA",
      description:
        "Criação assistida de propostas comerciais completas e personalizadas a partir de texto livre",
      icon: Sparkles,
      badge: "Inteligência Artificial",
      badgeColor:
        "bg-violet-50 text-violet-700 dark:bg-violet-950/40 dark:text-violet-400 border-violet-100 dark:border-violet-900/40",
    },
    {
      key: "clientExtract",
      name: "Extração de Leads / Clientes",
      description:
        "Identificação e cadastro automático de dados de contatos a partir de mensagens brutas de clientes",
      icon: UserPlus,
      badge: "Inteligência Artificial",
      badgeColor:
        "bg-violet-50 text-violet-700 dark:bg-violet-950/40 dark:text-violet-400 border-violet-100 dark:border-violet-900/40",
    },
    {
      ...
      description:
        "Redação profissional de escopos e descrições técnicas detalhadas para produtos ou serviços",
      icon: Wand2,
      badge: "Inteligência Artificial",
      badgeColor:
        "bg-violet-50 text-violet-700 dark:bg-violet-950/40 dark:text-violet-400 border-violet-100 dark:border-violet-900/40",
    },
    {
      ...
      description:
        "Enriquecimento de itens e sugestão inteligente de precificação para seu catálogo de serviços",
      icon: BookOpen,
      badge: "Inteligência Artificial",
      badgeColor:
        "bg-violet-50 text-violet-700 dark:bg-violet-950/40 dark:text-violet-400 border-violet-100 dark:border-violet-900/40",
    },
    {
      ...
      description:
        "Análise avançada de métricas do funil comercial, projeções de receita e recomendações acionáveis",
      icon: ReceiptText,
  ...
  const {
    ...
    refresh,
    status,
  } = useLazyFetch<any>("/api/dashboard/stats", {
    key: "dashboard-stats",
    query: fetchQuery,
    watch: [period],
  });
  ...
  const statusChartData = computed(() => {
    ...
    const statsSort = Object.fromEntries(
      Object.entries(stats.value.statusDistribution).sort(([a], [b]) =>
        a.localeCompare(b),
      ),
    );
    const labels = Object.keys(statsSort);
    const data = Object.values(statsSort) as number[];

    // Cor por segmento = cor da FASE do sistema (padrão único: proposalLifecycle)
    const colorByLabel: Record<string, string> = {
      // Fase 1 — Rascunho (gray)
      'Rascunho': getPhaseColorHex('draft'),
      'Criado': getPhaseColorHex('progress'),
      // Fase 2 — Em andamento (blue)
      'Enviado': '#3B82F6', 'Entregue': '#60A5FA', 'Aberto': '#93C5FD',
      'Clicado': '#BFDBFE', 'Visualizado': '#DBEAFE', 'Pendente': '#60A5FA',
      'Agendado': '#93C5FD', 'Recebido': '#BFDBFE', 'Atrasado': '#F59E0B',
      // Fase 3 — Assinatura (amber)
      'Aguardando Assinatura': '#F59E0B',
      // Fase 4 — Fechado (emerald)
      'Aceito': '#10B981', 'Assinado': '#10B981',
      // Fase 5 — Falhou (red)
      'Expirado': '#EF4444', 'Recusado': '#EF4444', 'Devolvido': '#EF4444',
      'Falha': '#EF4444', 'Suprimido': '#EF4444', 'Rejeitado': '#EF4444'
    };

    const backgroundColor = labels.map(
      (label) => colorByLabel[label] || getPhaseColorHex('progress'),
    );
      ...
      async () => {
        ...
        try {
          ...
          notify(
            ...
            res.message ||
              "Seu relatório estratégico está sendo gerado em segundo plano. Assim que estiver pronto, você será notificado na Central!",
          );
          ...
          if (e.statusCode === 402) {
            ...
            notify(
              "Limite Atingido",
              "Você fez muitas requisições seguidas. Tente novamente em um minuto.",
            );
          } else if (e.statusCode === 400) {
            notify(
              "Orçamento aprovado necessário",
              e.data?.statusMessage ||
                "É necessário ter pelo menos 1 orçamento aprovado para gerar um relatório estratégico.",
            );
          } else {
            notify(
              "Erro",
              e.data?.statusMessage || "Erro ao gerar relatório estratégico",
            );
          }
        } finally {
  ...
  const periodLabel = computed(
    () => periodLabels[period.value] || "Todo o período",
  );
  ...
  const tmaPercentage = computed(() => {
    if (!stats.value || !stats.value.tmaHours || periodDays.value <= 0)
      return 0;
