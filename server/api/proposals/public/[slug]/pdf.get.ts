...
export default defineEventHandler(async (event) => {
  ...
  // Processar variáveis
  const contractHtml = processVariables(proposal.contractText || '', proposal as any, profile as any)
  const termsHtml = processVariables(proposal.termsAndConditions || '', proposal as any, profile as any)

  // HTML Content (same as the private one, should ideally be shared)
  const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head>
